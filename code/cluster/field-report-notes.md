...
| Normal-vs-Weird depth | restored | Expanded operator-reality rows (service-state gating, storage/compute parity, launch ergonomics) plus dedicated `Weird / New / Interesting Findings` section were reintroduced from older report intent using canonical evidence. |
